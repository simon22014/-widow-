...
VIKBoardMain::VIKBoardMain(void)
{
}
VIKBoardMain::~VIKBoardMain(void)
{
...
void VIKBoardMain::Four_Point_init(Mat img)
{
	Mat imGG = img.clone();
	imshow("请点击i个点", imGG);
...
void VIKBoardMain::Four_Point_Past_init()
{
...
void VIKBoardMain::XY_warp_init(Mat img, Mat warp_mat)
{
	...
	for (i = 0;i<img.rows;i++)
	{
		...
		VIKBoardMain::XY_warp.push_back(A);
	}

}

vector<Point> VIKBoardMain::get_Four_Point_Past()
{
...
------------------------------------------*/
Mat VIKBoardMain::calculate_warp_mat(vector<Point> val, Mat img)
{
...
void VIKBoardMain::warp_transform(Mat img, Mat &src, Mat warp_mat, cv::Size _size)
{
	warpPerspective(img, src, warp_mat, img.size());
}

void VIKBoardMain::GetSkin(Mat &img)
{
...
void VIKBoardMain::GetSkin(Mat img, Mat &src)
{
...
void VIKBoardMain::transform_skin_warp(Mat &img)
{
	uchar *p;
	for (int i = 0; i < img.rows; ++i)
	{
...
------------------------------------------*/
void VIKBoardMain::area_of_key_init(Mat img)
{
	int a = 10;
...
------------------------------------------*/
void VIKBoardMain::draw_biankuang(Mat img)
{
...
int VIKBoardMain::tongji_skin(Mat img)
{
	int x, y, i;
	char word[16];
	uchar *p;
...
------------------------------------------*/
void VIKBoardMain::Get_Key_Value(char a[])
{
	a[0] = '`';
	a[1] = '1';
	a[2] = '2';
